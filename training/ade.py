...
def main():
  ...
  MODEL = 'ade' if args.model == 'distilled' else args.model
  ...
  for i in range(3):
    ...
    # Load the dataset
    train_data, val_data, test_data = load_data('ade')
    ...
    path = os.path.join('..', 'logs', 'ade', TRANSFER, MODEL, f'seed_{i}', 'mlm_org')
    ...
    # Load the model
    clf_org = get_clf(tune_args.output_dir, clf_args)
    ...
    path = os.path.join('..', 'logs', 'ade', TRANSFER, MODEL, f'seed_{i}', 'mlm_100')

    # Load the tokenizer
    tokenizer_100 = AutoTokenizer.from_pretrained(os.path.join('..', 'tokenizers', 'ade', 'ade_100'), model_max_length=SEQ_LEN)
    ...
    # Load the model
    clf_100 = get_clf(tune_args.output_dir, clf_args)
    shutil.rmtree(tune_args.output_dir)
    ...
    path = os.path.join('..', 'logs', 'ade', TRANSFER, MODEL, f'seed_{i}', 'mlm_75')

    # Load the tokenizer
    tokenizer_75 = AutoTokenizer.from_pretrained(os.path.join('..', 'tokenizers', 'ade', 'ade_75'), model_max_length=SEQ_LEN)
    ...
    # Load the model
    clf_75 = get_clf(tune_args.output_dir, clf_args)
    shutil.rmtree(tune_args.output_dir)
    ...
    path = os.path.join('..', 'logs', 'ade', TRANSFER, MODEL, f'seed_{i}', 'mlm_50')

    # Load the tokenizer
    tokenizer_50 = AutoTokenizer.from_pretrained(os.path.join('..', 'tokenizers', 'ade', 'ade_50'), model_max_length=SEQ_LEN)
    ...
    # Load the model
    clf_50 = get_clf(tune_args.output_dir, clf_args)
    shutil.rmtree(tune_args.output_dir)
    ...
    path = os.path.join('..', 'logs', 'ade', TRANSFER, MODEL, f'seed_{i}', 'mlm_25')

    # Load the tokenizer
    tokenizer_25 = AutoTokenizer.from_pretrained(os.path.join('..', 'tokenizers', 'ade', 'ade_25'), model_max_length=SEQ_LEN)
    ...
    # Load the model
    clf_25 = get_clf(tune_args.output_dir, clf_args)
    shutil.rmtree(tune_args.output_dir)
    ...
    analyser = CLFAnalyser({
        ...
        'ADE-100': (tokenizer_100, clf_100), 
        'ADE-75': (tokenizer_75, clf_75), 
        'ADE-50': (tokenizer_50, clf_50), 
        'ADE-25': (tokenizer_25, clf_25)
      }, X_test, y_test)
    ...
    # Save the statitics
    analyser.save_stats(os.path.join('..', 'logs', 'ade', TRANSFER, MODEL, f'seed_{i}'))
